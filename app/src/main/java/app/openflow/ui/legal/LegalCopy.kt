...
object LegalCopy {
    ...
    const val privacyIntro =
        "Last updated: 18 August 2026.\n\n" +
            "Open Flow is a free, local-first dictation app. No account. No ads. No analytics. " +
    ...
    val privacySections: List<Section> = listOf(
        ...
        Section(
            "If you pick a cloud ear or brain",
            "That vendor gets what that pick needs. We never send dictation history, dictionary, snippets, or other apps to an Open Flow server (there is none). Keys stay on the phone. Uninstall deletes them.",
        ),
        Section(
            "Your choices",
    ...
    val privacyBody: String get() = join(privacyIntro, privacySections)
    val termsBody: String get() = join(termsIntro, termsSections)

    private fun join(intro: String, sections: List<Section>): String = buildString {
        append(intro.trim())
        sections.forEach { s ->
            append("\n\n")
            append(s.title)
